...
from egttools.plotting.simplified import \
    plot_replicator_dynamics_in_simplex  # , plot_pairwise_comparison_rule_dynamics_in_simplex_without_roots
from egttools.plotting.helpers import (xy_to_barycentric_coordinates, barycentric_to_xy_coordinates,
                                       find_roots_in_discrete_barycentric_coordinates, calculate_stability)
...
class CRDWithExecutor():
    ...
    def __init__(self, strategies: list, initial_endowment, population_size: int, group_size: int, cost: float,
                 risk: float,
                 alpha: float, cooperation_threshold: int, enhancement_factor: float, pi_t: float, pi_e: float,
                 n_e: int, mu: int, incentive: tuple = ('local', 'fixed')):
        ...
        self.ce_rewards_d_fines = np.zeros((self.nb_strategies_, self.nb_states_))
    ...
    def defector_fixed_incentives_payoff(self, jc, je):
        return self.base_defector_payoff(jc + je) - self.defector_fixed_incentives_reward(je)

    def cooperator_fixed_incentives_payoff(self, jc, je):
        return self.base_defector_payoff(jc + je) + self.cooperator_fixed_incentives_reward(je) - self.c

    def executor_fixed_incentives_payoff(self, jc, je):
        return self.cooperator_fixed_incentives_payoff(jc, je) - self.pi_t

    def cooperator_fixed_incentives_reward(self,je):
        return self.alpha * self.pi_e * self.Delta(je)

    def executor_fixed_incentives_reward(self,je):
        return self.cooperator_fixed_incentives_reward(je)

    def defector_fixed_incentives_reward(self,je):
        return ((1 - self.alpha) * self.pi_e * self.Delta(je))
...
################################ Reward
    ...
    def calculate_payoffs(self) -> np.ndarray:
        ...
        for i in range(nb_states_):
            ...
            payoffs[1, i] = PI_E
            payoffs[2, i] = PI_C
        self.ce_rewards_d_fines = ce_rewards_d_fines
    ...
    def T(self, L, R, pop_idx):
        ...
        return (1 - self.mu) * (i_L / self.Z) * (i_R / (self.Z - 1)) * (
                    1 + math.e ** (-self.beta * (f_R - f_L))) ** -1 + self.mu * (i_L / (2 * self.Z))
    ...
    def aG(self, i):
        ...
        for jc in range(self.N + 1):
            for je in range(self.N - jc + 1):
                total += math.comb(ic, jc) * math.comb(ie, je) * math.comb(int(self.Z - ic - ie),
                                                                           self.N - jc - je) * theta(jc + je - self.M)
